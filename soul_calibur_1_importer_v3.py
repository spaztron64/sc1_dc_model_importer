...
def import_soul_calibur_model(filepath):
    with open(filepath, "rb") as file:
        ...
        for submesh in submeshes:
            ...
            # Convert bone position to floats and divide by 16000.0
            bone_translation = Vector((
                submesh["bone_position"][0] / 16000.0,
                submesh["bone_position"][1] / 16000.0,
                submesh["bone_position"][2] / 16000.0
            ))
            print(f'Bone Translation: {bone_translation}')
            ...
            for _ in range(submesh["vertex_count"]):
                ...
                # Apply the bone translation to each vertex
                transformed_vertex = Vector((vertex_x, vertex_y, vertex_z)) + bone_translation
                bm.verts.new(transformed_vertex)
                print(f"Original Vertex: ({vertex_x}, {vertex_y}, {vertex_z}) -> Transformed: {transformed_vertex}")
